...
| The algorithms and calculations were the same for all suppliers across the system.  A lot of data and algorithms were used to make calculations at any single moment.  Supplier data was rarely updated. | Cache data to memory rather than calling the database to get a current price.      |
| We needed to be mindful of cloud budget usage (it was a startup with a limited budget).  We needed to support limitless scaling (unlimited suppliers and buyers and unlimited transactions at any moment).       | Run the calculations concurrently using parallelism and multithreading.            |
